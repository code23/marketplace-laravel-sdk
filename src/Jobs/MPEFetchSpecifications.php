...
class MPEFetchSpecifications implements ShouldQueue
{
        ...
        if(MPECache::get('modules')->contains('product-specifications')) {
            ...
            if(!$oauth) {
                ...
                if($this->command) {
                    ...
                    if ($slack) SlackAlert::message('*' . config('app.url') . "* MPEFetchSpecifications: _Auth failed_");
                }

                Log::alert('MPEFetchSpecifications: Auth failed');

                throw new Exception('MPEFetchSpecifications: MPE Auth failed');
            }
