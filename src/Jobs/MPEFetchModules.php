...
class MPEFetchModules implements ShouldQueue
{
    ...
    */
    ...
    public function handle(): void
    {
        ...
        if($this->command) {
            ...
            $this->command->line('Fetching modules…');
        }
        ...
        if(!$oauth) {
            ...
            if($this->command) {
                ...
                if ($slack) SlackAlert::message('*' . config('app.url') . "* MPEFetchModules: _Auth failed_");
            }

            Log::alert('MPEFetchModules: Auth failed');

            throw new Exception('MPEFetchModules: MPE Auth failed');
        }
        ...
        Log::info('MPEFetchModules: Cached modules updated');

        if($this->command) {
            $this->command->info('Cached modules updated');
            $this->command->newLine();
        }
    }
}
