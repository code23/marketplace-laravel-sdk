...
class MPEFetchLatestPosts implements ShouldQueue
{
    ...
    */
    ...
    public function handle(): void
    {
        ...
        if($this->command) $this->command->line('Fetching latest posts…');
        ...
        if(!$oauth) {
            ...
            if($this->command) {
                ...
                if ($slack) SlackAlert::message('*' . config('app.url') . "* MPEFetchLatestPosts: _Auth failed_");
            }

            Log::alert('MPEFetchLatestPosts: Auth failed');

            throw new Exception('MPEFetchLatestPosts: MPE Auth failed');
        }

        // get params from config
        $params = config('boilerplate.mpe_cache.latest_posts.params');

        try {
            // get posts from API
            $posts = MPEBlog::posts($params, $oauth)['data'];

            // if none returned
            if(! count($posts) && $this->command) {
                $this->command->error('Latest posts data empty');
            }
        } catch (Exception $e) {
        ...
        // update cached data
        Cache::put('latest_posts', $posts);

        if($this->command) {
            $this->command->info('Cached latest posts updated');
